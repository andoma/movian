...
lircd_proc(hts_prop_t *status)
{
  ...
  if((fd = open(dev, O_RDONLY)) == -1) {
    hts_prop_set_stringf(status, "lirc: Unable to open \"%s\"", dev);
    sleep(1);
    return;
  }
  ...
  while(hid_ir_mode == HID_IR_LIRC) {
	...
	hts_prop_set_stringf(status, "lirc: Unable to read from \"%s\"", dev);
	break;
      }
    ...
    while((len = htsbuf_find(&q, 0xa)) != -1) {
      ...
      if(len >= sizeof(buf) - 1) {
	hts_prop_set_stringf(status, "lirc: Command buffer size exceeded");
	goto out;
      }
