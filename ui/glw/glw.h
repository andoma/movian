...
typedef enum {
  ...
  GEV_EXT_BASE,        /* Application specific entries starts here */
} glw_event_type_t;

TAILQ_HEAD(glw_event_q, glw_event);

typedef struct glw_event_queue {
  struct glw_event_q geq_q;
  hts_cond_t geq_cond;
  hts_mutex_t geq_mutex;
} glw_event_queue_t;

typedef struct glw_event {
  int     ge_refcnt;
  int     ge_mapped;
  glw_event_type_t ge_type;
  void (*ge_dtor)(struct glw_event *ge);
  TAILQ_ENTRY(glw_event) ge_link;
} glw_event_t;

typedef struct glw_event_unicode {
  glw_event_t h;
  int sym;
} glw_event_unicode_t;

typedef struct glw_event_sys {
  glw_event_t h;
  char *target;
  char *method;
  char *argument;
...
typedef struct glw_event_map {
  glw_event_type_t gem_inevent;
  glw_event_type_t gem_outevent;
  char *gem_target;
  char *gem_method;
  char *gem_argument;
...
void *glw_event_create(glw_event_type_t type, size_t size);

void *glw_event_create_unicode(int sym);

void glw_event_enqueue(glw_event_queue_t *geq, glw_event_t *ge);

glw_event_t *glw_event_get(int timeout, glw_event_queue_t *geq);

void glw_event_unref(glw_event_t *ge);

void glw_event_initqueue(glw_event_queue_t *geq);

void glw_event_flushqueue(glw_event_queue_t *geq);

void glw_event_signal_simple(glw_t *w, glw_event_type_t type);

/**
 * Higher level functions
 */
void glw_get_caption(glw_t *w, const char *id, char *buf, size_t buflen);

int glw_get_model(glw_t *w, const char *id, char *buf, size_t buflen);

void glw_switch_model(glw_t *w, const char *id, const char *model);

int glw_event_enqueuer(glw_t *w, void *opaque, glw_signal_t sig, void *extra);

glw_event_t *glw_wait_form(glw_t *root);

int glw_wait_form_ok_cancel(glw_t *root);

glw_t *glw_selection_add_text_option(glw_t *opt, const char *caption,
				     void (*cb)(void *opaque, void *opaque2, 
						int value),
				     void *opaque, void *opaque2, 
				     int value, int selected);

glw_t *glw_selection_get_widget_by_opaque(glw_t *parent, void *opaque);
