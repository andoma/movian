 ...
 */
void
glw_tex_render_setup(glw_tex_renderer_t *gtr, int quads)
{
  gtr->gtr_buffer = malloc(sizeof(float) * 5 * 4 * quads);
  gtr->gtr_quads  = quads;
}


void
glw_tex_render_free(glw_tex_renderer_t *gtr)
{
  free(gtr->gtr_buffer);
}
...
void
glw_tex_render_set_vertex(glw_tex_renderer_t *gtr, int vertex,
			  float x, float y, float z, float s, float t)
{
  gtr->gtr_buffer[vertex * 5 + 0] = x;
  gtr->gtr_buffer[vertex * 5 + 1] = y;
  gtr->gtr_buffer[vertex * 5 + 2] = z;
  gtr->gtr_buffer[vertex * 5 + 3] = s;
  gtr->gtr_buffer[vertex * 5 + 4] = t;
}


void
glw_tex_render(glw_tex_renderer_t *gtr, glw_backend_texture_t *be_tex,
	       float r, float g, float b, float a)
{
  int i;
  float *buf = gtr->gtr_buffer;
  ...
  for(i = 0; i < gtr->gtr_quads * 4; i++) {

    glTexCoord2f(buf[i * 5 + 3],
		 buf[i * 5 + 4]);

    glVertex3f(buf[i * 5 + 0],
	       buf[i * 5 + 1],
	       buf[i * 5 + 2]);
  }
