			...
			size_t filenamelen, struct svfs_ops *svfs_ops) 
{
    ...
    while( ( ent = readdir( dir ) ) != NULL ) {
        if( !strcasecmp( ent->d_name, file ) ) {
            snprintf( filename, filenamelen, "%s%s%s", path,
                     ( ( path[ strlen( path ) - 1 ] == '/' ) ? "" : "/" ),
                     ent->d_name );
            closedir( dir );
            return 0;
 ...
 */
static dvd_file_t *DVDOpenFilePath( dvd_reader_t *dvd, char *filename, struct svfs_ops *svfs_ops )
{
...
static dvd_file_t *DVDOpenVOBPath( dvd_reader_t *dvd, int title, int menu, struct svfs_ops *svfs_ops )
{
    char filename[ MAX_UDF_FILE_NAME_LEN ];
    char full_path[ PATH_MAX + 1 ];
    struct stat fileinfo;
