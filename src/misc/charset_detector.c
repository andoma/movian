...
static const ngram_lang_t ngrams_windows_1251[] = {
  {
    {
...
static const ngram_lang_t ngrams_windows_1256[] = {
  {
    {
...
static const ngram_lang_t ngrams_8859_1[] =  {
  { 
    {
...
static const ngram_lang_t ngrams_8859_2[] =  {
  {
    {
...
static const ngram_lang_t ngrams_8859_5[] =  {
  {
    {
...
static const ngram_lang_t ngrams_8859_6[] =  {
  {
    {
...
static const ngram_lang_t ngrams_8859_7[] = {
  {
    {
...
static const ngram_lang_t ngrams_8859_8[] = {
  {
    {
...
static const ngram_lang_t  ngrams_8859_9[] = {
  {
    {
...
#define NG(x) x, ARRAY_SIZE(x)

static const struct {
  const char *encoding;
  const char *encoding_alt;
  const uint8_t *map;
  const ngram_lang_t *ngram_lang;
  int ngram_lang_size;
} detector_table[] = {
  { "ISO-8859-1", "windows-1252", charMap_8859_1, NG(ngrams_8859_1) },
  { "ISO-8859-2", "windows-1250", charMap_8859_2, NG(ngrams_8859_2) },
  { "ISO-8859-5", NULL,           charMap_8859_5, NG(ngrams_8859_5) },
  { "ISO-8859-6", NULL,           charMap_8859_6, NG(ngrams_8859_6) },
  { "ISO-8859-7", "windows-1253", charMap_8859_7, NG(ngrams_8859_7) },
  { "ISO-8859-8", "windows-1255", charMap_8859_8, NG(ngrams_8859_8) },
  { "ISO-8859-9", "windows-1254", charMap_8859_9, NG(ngrams_8859_9) },
  { "windows-1256", NULL,  charMap_windows_1256, NG(ngrams_windows_1256) },
  { "windows-1251", NULL,  charMap_windows_1251, NG(ngrams_windows_1251) },
};
 ...
 */
static int
ngram_search(const int32_t *table, int32_t value)
{
    int32_t index = 0;
...
parse_ngram(const int32_t *n, const uint8_t *map, const uint8_t *str, int len)
{
  ...
  for(int i = 0; i < len; i++) {
    ...
    if(mb) {
      if(!(mb == 0x20 && ign_spc)) {
	ngram = ((ngram << 8) + mb) & N_GRAM_MASK;
	ngram_count++;
	int res = ngram_search(n, ngram);
	if(res >= 0)
	  ngram_hit++;
      }
      ign_spc = mb == 0x20;
  ...
  return (float)ngram_hit / (float)ngram_count;
}
...
charset_detector(const uint8_t *str, int len, const char **lp)
{
  ...
  for(int i = 0; i < ARRAY_SIZE(detector_table); i++) {

    const ngram_lang_t *nl = detector_table[i].ngram_lang;

    for(int j = 0; j < detector_table[i].ngram_lang_size; j++) {
	...
	if(c1bytes && detector_table[i].encoding_alt)
	  best = detector_table[i].encoding_alt;
	best_score = c;
	if(lp != NULL)
	  *lp = lang;
      }
