 ...
 */
/*void
prop_print_tree_json0(prop_t *p, htsbuf_queue_t *hq, int indent, int followlinks, int index)
{
  prop_t *c;
  char buf[200];
  int type = p->hp_type;
  ...
  if(p->hp_originator != NULL) {
    if(followlinks) {
      if (p->hp_originator->hp_originator == NULL) {
snprintf(buf, sizeof(buf), "aaa");
  htsbuf_append(hq, buf, strlen(buf));        
prop_print_tree_json0(p->hp_originator, hq, indent, followlinks, 0);
return;
      }
      else {
snprintf(buf, sizeof(buf), "bbb");
  htsbuf_append(hq, buf, strlen(buf));   
        prop_print_tree_json0(p->hp_originator, hq, indent, followlinks, 0);
      }
    }
...
snprintf(buf, sizeof(buf), "%*.s{}", indent, "");
  ...
  if (p->hp_name != NULL) {
    snprintf(buf, sizeof(buf), "\"%s\": ", p->hp_name);
    htsbuf_append(hq, buf, strlen(buf));
  }

  switch(type) {
  case PROP_RSTRING:
    htsbuf_append_and_escape_jsonstr(hq, rstr_get(p->hp_rstring));
    break;

  case PROP_CSTRING:
    htsbuf_append_and_escape_jsonstr(hq, p->hp_cstring);
    break;

  case PROP_LINK:
    snprintf(buf, sizeof(buf), "%s <%s>", rstr_get(p->hpn_link_rtitle),
	    rstr_get(p->hpn_link_rurl));
    htsbuf_append_and_escape_jsonstr(hq, buf);
    break;

  case PROP_FLOAT:
    my_double2str(buf, sizeof(buf), p->hp_float);
    htsbuf_append(hq, buf, strlen(buf));
    break;
  ...
  case PROP_DIR:
    snprintf(buf, sizeof(buf), "%s\n", (TAILQ_FIRST(&p->hp_childs) != NULL && TAILQ_FIRST(&p->hp_childs)->hp_name == NULL) ? "[" : "{");
    htsbuf_append(hq, buf, strlen(buf));

    TAILQ_FOREACH(c, &p->hp_childs, hp_parent_link) {
      prop_print_tree_json0(c, hq, indent + 4, followlinks, 0);
    }

    snprintf(buf, sizeof(buf), "%*.s%s", indent, "", (TAILQ_FIRST(&p->hp_childs) != NULL && TAILQ_FIRST(&p->hp_childs)->hp_name == NULL) ? "]" : "}");
    htsbuf_append(hq, buf, strlen(buf));
    break;
  ...
  case PROP_ZOMBIE:
    snprintf(buf, sizeof(buf), "\"<zombie, ref=%d>\"", p->hp_refcount);
    htsbuf_append(hq, buf, strlen(buf));
    break;
  }
  }

  if(TAILQ_NEXT(p, hp_parent_link)) {
