...
get_anon_file(void)
{
  int fd = open(cachefile, O_CREAT | O_TRUNC | O_RDWR, 0666);
  if(fd != -1 && unlink(cachefile)) {
    close(fd);
    fd = -1;
  }
  return fd;
...
fac_load(cached_file_t *cf, void *buf, int64_t offset, int size)
{
  ...
  if(lseek(cf->cf_fd, voff, SEEK_SET) != voff)
    return -1;
  if(read(cf->cf_fd, buf, count) != count)
    return -1;
...
cache_pages(cached_file_t *cf, void *buf)
{
  ...
  for(i = 0; i < cf->cf_num_pages - 10; i++) {
    ...
    if(r == -1)
      break;
    if(r == 1 && 0)
      TRACE(TRACE_DEBUG, "Read-ahead", "%d pages", ra);
    ra++;
  }
}
...
{
  fa_handle_t *fh = fa_open(url, errbuf, errsize, flags | FA_HUGE_BUFFER);
  if(fh == NULL)
    return NULL;
