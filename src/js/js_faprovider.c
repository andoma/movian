        ...
        char *errbuf, size_t errsize, int non_interactive)
{
  ...
  int r = JS_CallFunctionValue(cx, NULL, statfn, 2, argv, &result);
  JS_PopArguments(cx, mark);
  if(!r) {
  ...
  if(JSVAL_IS_OBJECT(result)) {
    ...
    buf->fs_type = js_prop_int_or_default(cx, o, "isDir", 0) ?
      CONTENT_DIR : CONTENT_FILE;
    buf->fs_mtime = js_prop_int_or_default(cx, o, "mtime", 0);
    r = 0;
  } else {
    r = -1;
  }
