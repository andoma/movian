...
htsp_channelAddUpdate(htsp_connection_t *hc, htsmsg_t *m, int create)
{
  ...
  prop_set_string(prop_create(media, "type"), "tvchannel");
  prop_set_string(prop_create(media, "icon"), 
		  htsmsg_get_str(m, "channelIcon"));
  prop_set_string(prop_create(media, "title"),
		  htsmsg_get_str(m, "channelName"));


  if(create)
    prop_set_parent(p, hc->hc_prop_channels);
}
...
htsp_thread(void *aux)
{
  ...
  while(1) {
    ...
    while(1) {
      ...
      if(tcp_read(hc->hc_fd, &msglen, 4))
	break;

      msglen = ntohl(msglen);
      msgbuf = malloc(msglen);
      
      if(tcp_read(hc->hc_fd, msgbuf, msglen)) {
	free(msgbuf);
	break;
      }
      
      if((m = htsmsg_binary_deserialize(msgbuf, msglen, msgbuf)) == NULL)
	break;
