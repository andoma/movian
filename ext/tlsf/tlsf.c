...
fail(void *ptr)
{
	TRACE(TRACE_ERROR, "DUMP", "Hexdump from %p - 0x20 + 0x20",
	    ptr);
	hexdump("FAIL", ptr - 0x20, 0x40);
	assert(0);
}
...
static void integrity_walker(void* ptr, size_t size, int used, void* user)
{
	...
	if(integ->prev_status != this_prev_status) {
		TRACE(TRACE_ERROR, "TLSF", "%p ps %d != %d", ptr, 
		    integ->prev_status, this_prev_status);
		fail(ptr);
	}

	if(size != this_block_size) {
		TRACE(TRACE_ERROR, "TLSF", "%p size %d != %d", ptr, 
		    integ->prev_status, this_prev_status);
		fail(ptr);
	}
