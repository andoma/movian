...
static hts_thread_t	malloc_initializer;

/* Used to avoid initialization races. */
MALLOC_MUTEX_INITIALIZE(static, init_lock);
...
malloc_init_hard(void)
{
	...
	if (malloc_initialized || malloc_initializer == hts_thread_current()) {
		/*
...
#error pip
	...
	if (hts_thread_key_create(&arenas_tsd, arenas_cleanup) != 0) {
		malloc_mutex_unlock(&init_lock);
		return (true);
	}
	...
	/* Get number of CPUs. */
	malloc_initializer = hts_thread_current();
	malloc_mutex_unlock(&init_lock);
	ncpus = malloc_ncpus();
	malloc_mutex_lock(&init_lock);
