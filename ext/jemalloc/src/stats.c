    ...
    unsigned i)
{
	...
	for (j = 0, gap_start = UINT_MAX; j < nbins; j++) {
		...
		if (nruns == 0) {
			...
			if (config_tcache) {
				malloc_cprintf(write_cb, cbopaque,
				    "%13u %1s %5lu %4u %3lu %12lu %12"PRIu64
	...
	for (j = 0, gap_start = -1; j < nlruns; j++) {
		...
		if (nrequests == 0) {
			...
			CTL_IJ_GET("stats.arenas.0.lruns.0.curruns", &curruns,
			    size_t);
			if (gap_start != -1) {
				malloc_cprintf(write_cb, cbopaque, "[%lu]\n",
				    j - gap_start);
				gap_start = -1;
			}
			malloc_cprintf(write_cb, cbopaque,
			    "%13lu %5lu %12"PRIu64" %12"PRIu64" %12"PRIu64
			    " %12lu %12lu\n",
    ...
    unsigned i)
{
	...
	malloc_cprintf(write_cb, cbopaque,
	    "dirty pages: %lu:%lu active:dirty, %"PRIu64" sweep%s,"
	...
	malloc_cprintf(write_cb, cbopaque,
	    "small:   %12lu %12"PRIu64" %12"PRIu64" %12"PRIu64"\n",
	...
	malloc_cprintf(write_cb, cbopaque,
	    "large:   %12lu %12"PRIu64" %12"PRIu64" %12"PRIu64"\n",
	    large_allocated, large_nmalloc, large_ndalloc, large_nrequests);
	malloc_cprintf(write_cb, cbopaque,
	    "total:   %12lu %12"PRIu64" %12"PRIu64" %12"PRIu64"\n",
	    small_allocated + large_allocated,
	    small_nmalloc + large_nmalloc,
	    small_ndalloc + large_ndalloc,
	    small_nrequests + large_nrequests);
	malloc_cprintf(write_cb, cbopaque, "active:  %12lu\n",
	    pactive * pagesize );
	CTL_I_GET("stats.arenas.0.mapped", &mapped, size_t);
	malloc_cprintf(write_cb, cbopaque, "mapped:  %12lu\n", mapped);
...
#undef OPT_WRITE_CHAR_P
...
#ifdef JEMALLOC_STATS
	{
		...
		malloc_cprintf(write_cb, cbopaque,
		    "Allocated: %lu, active: %lu, mapped: %lu\n",
		    allocated, active, mapped);
		malloc_cprintf(write_cb, cbopaque,
		    "Current active ceiling: %lu\n", atomic_read_z(cactive));
